...
device_suggester_agent = Agent(
    name="device_suggester_agent",
    model=MODEL_GEMINI_2_0_FLASH,
    description="Agent to suggest energy-efficient devices and decide on a micro-credit offer.",
    instruction='''
...
- The type of bill (electricity, gas, or water).
- Their average monthly consumption (e.g., in kWh for electricity).

Here is your process:
1. Greet the user and explain your purpose.
2. Ask for the type of bill they want to analyze.
3. Ask for their average monthly consumption.
4. Once you have all the information, output a JSON object with the collected data. The keys must be "bill_type" and "consumption".

After having all the required information, delegate to: 'device_suggester_agent'
''',
    sub_agents=[device_suggester_agent],
